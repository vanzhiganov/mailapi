...
class NoSuchMailbox(Exception):
    ...
    def __init__(self, email_address):
        error_message = 'A mailbox with the email address %s does not exist.' % email_address
...
class MailboxExists(Exception):
    ...
    """
    def __init__(self, email_address):
        error_message = 'A mailbox with the email address %s already exists.' % email_address
...
class DbInitError(Exception):
    ...
    """
    def __init__(self):
        super(DbInitError, self).__init__('You must call init_db before making any API calls.')
...
class AliasExists(Exception):
    ...
    def __init__(self, source, dest):
        error_message = 'The given alias (%s --> %s) already exists.' % (source, dest)
